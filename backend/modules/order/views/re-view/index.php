...
Modal::begin([
    ...
    'header' => '<h4 class="modal-title">批量审核</h4>',
    'footer' => '',
    'options' => [
        'tabindex' => false
    ]
...
?>
<?php
$request_batch_update_url = Url::to(['batch-re-view']);
$modal_js = <<<JS
 $('#data-batch-update').bind('click', function () {
       var keys = $("#grid").yiiGridView("getSelectedRows");
       if(keys.length>0){
           $('#batch-update-modal').modal('toggle')
            $('#batch-update-modal').find('.modal-header').html('<h4 class="modal-title">批量审核</h4>');
