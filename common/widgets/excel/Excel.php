...
namespace common\widgets\excel;
...
class Excel extends \yii\base\Widget
{
    ...
    /**
     * @var  array 导入或者导出的首行标题，当setFirstRecordAsKeys为true时使用
     */
    public $ioTitle;
    ...
    {
        $temp_keys = ArrayHelper::remove($sheetData, '1');
        $keys = $temp_keys;
        foreach ($temp_keys as $key=>$value)
        {
            $keys[$key] = array_search(trim($value), $this->ioTitle);
        }
        $new_data = [];
