...
//            'data' => isset($serial_brand_data)?$serial_brand_data:[],
        ...
        ?>
        <?=  $form->field($model, 'promotion_detail_id')->label('选择品牌券')->widget(Select2::classname(), [
            'options' => ['placeholder' => '请输入品牌券名称...'],
//            'data' => isset($serial_brand_data)?$serial_brand_data:[],
            ...
            'pluginOptions' => [
                ...
                'ajax' => [
                    ...
                    'data' => new JsExpression('
                    function(params) { 
                        return {promotion_detail_name:params.term,type:3}; 
                    }
                    ')
                ],
                'escapeMarkup' => new JsExpression('function (markup) { return markup; }'),
        ...
        ?>
        <?=  $form->field($model, 'serial_id')->label('选择品牌期')->widget(Select2::classname(), [
            'options' => ['placeholder' => '请输入期咨询名称...'],
//            'data' => isset($serial_brand_data)?$serial_brand_data:[],
            'data' => $model->serialArrayData,
            'pluginOptions' => [
                'allowClear' => true,
                'ajax' => [
                    'url' => \yii\helpers\Url::to(['/content/serial/search-serial']),
                    'dataType' => 'json',
                    'data' => new JsExpression('
                    function(params) { 
                        var promotion_detail_id = $("#tglink-promotion_detail_id option:selected").val();
                        if(!promotion_detail_id){
                            $(".field-tglink-promotion_detail_id ").addClass("has-error").find(".help-block").html("请选择品牌券");
                        }else{
                            return {serial_name:params.term,promotion_detail_id:promotion_detail_id};
                        }
                    }
                    ')
                ],
